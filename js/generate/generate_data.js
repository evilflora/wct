...
function mapBobbleHead(raw) {
	if(BOBBLEHEAD_EXCLUDE_END.some(element => raw.uniqueName.endsWith(element))) return[];
...
function mapGlyph(raw) {
	if(GLYPH_EXCLUDE_END.some(element => raw.uniqueName.endsWith(element))) return[];
...
function mapMod(raw) { // I know it's a fuc**ng mess
	...
	if(raw.uniqueName.startsWith("/Lotus/Upgrades/Focus")) return mapFocusSchool(raw); // ||raw.uniqueName.startsWith("/Lotus/Upgrades/Mods/Sets/")
...
function mapSkin(raw) {
	...
	else if(raw.uniqueName.includes("TennoCon") || raw.uniqueName.includes("Tennocon")) return []; // todo need check
